...
function Register(): JSX.Element {
  ...
  async function send(user: UserModel) {
    try {
      const isTaken = await authService.emailIsTaken(user.email);
      if (isTaken) {
        setIsTaken(true);
        return;
      } else {
              ...
              <Typography variant="body2">
                <input
                  ...
                  {...register("email", {
                    ...
                    minLength: {
                      value: 4,
                      message: "Email must be inclode at least 4 chars",
                    },
                    maxLength: {
                      value: 100,
                      message: "Email can't be over 100 chars",
                    },
                  })}
                />
                {(formState.errors.email?.message && (
                  ...
                  (isTaken && <span>email is taken</span>)}
